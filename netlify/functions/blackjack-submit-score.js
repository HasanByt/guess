export default async (req) => {
  try {
    ...
    if (!Number.isInteger(cleanScore) || cleanScore < 0 || cleanScore > 999999) {
      return new Response(
        JSON.stringify({ error: "Ungültiger Score (0-999999)." }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    const key = "blackjackScore";
    const member = `${cleanName}::${cleanScore}`;

    // Höchstes Guthaben = besser, bei Gleichstand gewinnt der neueste
    ...
    if (!r.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Upstash error", details: t }),
